...
(cd Python-2.7.7 && ./configure --prefix=$LOCAL && make && make install)
rm -rf Python-2.7.7

tar xzvf setuptools-5.2.tar.gz
(cd setuptools-5.2 && $LOCAL/bin/python setup.py install)
rm -rf setuptools-5.2

tar xzvf pip-1.5.6.tar.gz
(cd pip-1.5.6 && $LOCAL/bin/python setup.py install)
rm -rf pip-1.5.6
